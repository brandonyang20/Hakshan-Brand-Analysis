...
**Version:** 1.1  
**Date:** May 2026 (updated 2026-05-23 — post office-hours session)
**Status:** Active  
...
>
> **v1.1 changes:** Dashboard is the product (not WhatsApp delivery). Hakshan updated to 13 locations. ICP sharpened around Bai Wei Mini Bowl archetype. GTM Month 1 rewritten around Bai Wei as first non-founder customer. WhatsApp/email delivery repositioned as Phase 2B feature.
...
The one-line pitch: *"Every branch. Every metric. One dashboard — so you know what's happening before it becomes a problem."*

A single login, a single view across all branches: reviews, ratings, social, and alerts. Priced for Malaysian SMEs.

---

## 2. Problem

Multi-branch F&B owners (3–13 outlets) are managing brand health manually across:
- 3–13 Google Business profiles
...
**Why now:**
- Hakshan (13-location Hakka chain — 10 restaurants + 3 cloud kitchens — Klang Valley) is the founder's own restaurant. This tool was built for them first — founder-market fit is real.
...
| Outlet count | 3–13 branches |
| Geography | Klang Valley (MY) → expand to SG, Penang, Johor Bahru |
| Team size | No dedicated marketing person |
| Decision maker | Founder or ops director — they can say yes or no to a subscription on the spot |
| Pain level | High — missing brand health signals across branches until damage is already done |
| Budget | RM200–400/month is negligible if it prevents one crisis |
| Communication style | WhatsApp-first; checks phone before laptop |

**Archetype customer: Bai Wei Mini Bowl founder.** Independent F&B chain, Klang Valley, founder-run, no central marketing team. The Bai Wei founder saw the BrandPulse prototype and offered to invest — the strongest possible demand signal. This is the profile to optimise for, not large chains.

**Counter-ICP (do not target):**
- Single-outlet F&B — not enough data to make the product useful
- Chains where head office manages digital presence centrally (e.g. Old Town White Coffee, corporate franchises) — the individual operator has no pain and no control; decisions are made by HQ
...
### 4.1 The Wedge: Multi-Branch Dashboard

**The product is the dashboard.** A single login where the founder can see every branch's Google rating, review count and delta, Instagram followers, Facebook likes, and week-over-week trends — all on one screen.

This was the founder's original vision and the thing that caused Hakshan's CEO, shareholders, and other F&B founders to immediately say "I need this." It is not a report. It is not a WhatsApp message. It is a single source of truth.

**What "ready to charge" means:**
- Owner can log in
- They see all their branches as comparable cards (ratings, review count, deltas, social counts)
- The data is live and updates weekly
- Nothing else is required

**Why the dashboard is the wedge (not WhatsApp delivery):**
- The prototype got an investment offer before WhatsApp delivery was ever mentioned
- Operators want to pull information on their own schedule, not be pushed a summary on ours
- Dashboard access creates a daily/weekly habit; a WhatsApp message is easy to ignore
- Login = measurable engagement (sessions, time on page); WhatsApp open rate is unmeasurable

**Note on WhatsApp delivery:** This feature was an AI suggestion (not the founder's original idea) that appeared in earlier planning documents. It is a valid Phase 2B feature — a push notification to alert operators when something needs attention — but it is not the product and not the wedge. Do not let it drive architecture decisions in Phase 1.

**Technical prerequisite:** The weekly review delta requires a time-series review snapshot service — a weekly cron job that fetches and stores review counts per branch. This does not currently exist and is a Phase 1B build requirement.

### 4.2 Upsell 1: Dashboard Drill-Down + Trend History

The wedge dashboard shows the current week's snapshot. The Insight tier adds:
- Per-branch drill-down (review themes, sentiment, individual reviews)
- Week-over-week trend charts (6-month history)
- Competitor side-by-side comparison

Pulse gets the summary view (all branches, totals, current week). Insight gets full drill-down with trend history and competitor module.
...
|---|---|---|---|---|
| **Pulse** | RM199/month | RM1,990/year *(RM166/month)* | RM398 (2 months free) | Multi-branch summary dashboard (all branches, ratings, review deltas, social counts, weekly alerts) |
...
**No free trial.** The sales demo IS a live dashboard walkthrough — the founder sets up the prospect's brands in the app and shows them their own real data. Once they see their own branches on screen, they subscribe. Payment starts on day one.
...
**Goal:** Get first non-founder customer using a live dashboard. First priority: Bai Wei Mini Bowl.

| Week | Action |
|---|---|
| 1 | Contact Bai Wei Mini Bowl founder. Get their branch details (Google Maps URLs, IG handle, Facebook page). Add them as a second hardcoded tenant at `/t/baiweiminibowl/dashboard`. Give them access. |
| 2 | Let Bai Wei founder use the dashboard for one week. Ask: "What would you miss if we turned it off?" Send Billplz bill for RM199/month. |
| 3 | Second paying customer from Hakshan's F&B network (supplier, adjacent chain). Demo: show them their own branches live in the app. |
| 4 | Third paying customer. Target 3 paying customers by end of Month 1. |

**Founder bandwidth constraint:** Hakshan now has 13 locations (10 restaurants + 3 cloud kitchens). BrandPulse sales and build time compete directly with operations. Weeks 1–4 must be scoped to tasks executable in ~5 hours/week.
...
|---|---|---|
| **Founder bandwidth** — running 13 locations (10 restaurants + 3 cloud kitchens) + building SaaS | **Very High** | Cap GTM at 5 hrs/week; hire ops support for Hakshan before scaling BrandPulse past 10 customers |
